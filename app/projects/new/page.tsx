...
import { Upload, FileText, Settings, BrainCircuit, Scissors, Wand2, ArrowRight, ArrowLeft } from "lucide-react";
import { useRouter } from "next/navigation";
import { EmbeddingModel } from "@prisma/client";
import Link from "next/link";

type ChunkingStrategy = "simple" | "mistral_ocr" | "contextualized" | "recursive" | "semantic";
...
export default function NewProjectPage() {
    ...
    const [progress, setProgress] = useState<{ status: string, chunks: Document[] }>({ status: "Starting...", chunks: []});

    // Template-specific state
    const [selectedTemplate, setSelectedTemplate] = useState<ChunkingStrategy>("recursive");
                ...
                for (const line of eventLines) {
                    try {
                        ...
                        if (eventData.type === 'status') {
                            ...
                            setProgress(prev => ({ ...prev, chunks: [...prev.chunks, eventData.data] }));
                        ...
                        <div className="overflow-y-auto p-4 text-left">
                            {progress.chunks.map((chunk, index) => (
                                <div key={index} className="text-xs font-mono border-b border-gray-700 py-2 animate-fade-in">
                                    ...
                                    <span className="text-gray-300">{chunk.content.substring(0, 100)}...</span>
                                </div>
                            ))}
                        </div>
                    </div>
